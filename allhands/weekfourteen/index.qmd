...
complexity of this table is $O(n²)$. The experimental results confirm the
...
complexity was $O(n \times log_2(n))$. Compared to bubble sort and insertion sort the
growth of this function was a bit slow. Which makes sense that it had shorter
run times.
...
function might have a bad Big O(n) notation it is important to consider the time
differences that still exist between functions with the same Big O(n) notation.
