...
title: Following a doubling experiment paradigm how does the response time for operations like append and concatenation vary for different sizes of data between mutable and immutable containment structures
page-layout: full
...
For our All Hands Project, we are examining mutable and immutable data structures—specifically lists and tuples. Our focus is on analyzing how appending elements to a list and concatenating elements to a tuple impact runtime performance.
...
**Time Complexity:**
- Worst Case: O(n) where n is list size

**Performance Characteristics:**
The implementation's efficiency stems from Python's list management system. When appending elements, Python works directly with the original data structure, avoiding unnecessary copies. This approach significantly reduces memory allocation overhead since no new list creation is needed for each operation.
...
**Time Complexity:**
- Worst Case: O(m * n) where m is number of sublists and n is sublist size
...
**Time Complexity:**
- Worst Case: O(m * n) where m is number of inner tuples and n is inner tuple size
...
Our analysis reveals four distinct performance patterns based on the fundamental differences between Python's mutable and immutable data structures:

1. Base List Operations
...
3. Nested List Operations
Pattern: Higher overhead but maintains stability with more frequent spikes
