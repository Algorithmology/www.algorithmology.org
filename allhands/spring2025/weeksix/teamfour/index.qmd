...
For our All Hands Project, we are examining mutable and immutable data
structures—specifically lists and tuples. Our focus is on analyzing how
appending elements to a list and concatenating elements to a tuple impact
runtime performance.
...
The implementation's efficiency stems from Python's list management system. When
appending elements, Python works directly with the original data structure,
avoiding unnecessary copies. This approach significantly reduces memory
allocation overhead since no new list creation is needed for each operation.
