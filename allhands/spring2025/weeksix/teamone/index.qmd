...
title: How is runtime affected in a vertical and horizontal subclassing hierarchy?
page-layout: full
...
In our experiment, we wanted to see if making a class hierarchy deeper (more
levels of inheritance) would slow down a program in Python. We thought that if a
class had to go through many layers to find a function, it would take more time.
But when we tested it, we found out that this wasn't really true.  
...
each other. For example: Shape (base class), Circle, Square, Triangle (all of
them coming from the base class 'Shape'). 
