...
Our experiments demonstrate that `binary search` algorithms (`iterative`, `recursive`) and BST consistently outperform `linear search` and it takes longer, especially as the dataset grows, whereas` binary search` and `BST` search times remain relatively low and constant.
...
- **Unbalanced BST Performance**
    -  Examine how performance degrades when BSTs become unbalanced, and compare with tree-balancing algorithms
