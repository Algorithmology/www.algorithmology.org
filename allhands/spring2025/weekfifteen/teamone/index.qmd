...
The ListProcessor class implements a tree data structure using Python's list to store parent-child relationships. Each relationship is stored as a tuple pair (parent, child) within the list, allowing for efficient storage and traversal of the tree structure.
...
| Set            | 0(1)avg  | 0(1)avg  | 0(1)avg  | 0(1)av            |
...
- Set is faster constantly for the both small and large data sizes compared to list as list
...
Set is alot faster than list because it uses hash table while list use dynamic array. Other than append(insert) which amortize 0(1) due to python geometrically growing capacity, List must look at every element or need to shift for other operations which results in 0(n).
