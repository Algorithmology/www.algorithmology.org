...
- Can we make the `Queue` implementation more efficient?
    ...
    - Accept that this will increase the space overhead ?!?!
...
::: {.fragment .fade-left style="margin-top: 0.2em; font-size: 0.9em;"}

- `ListQueueFakeDelete` increments the `_head` index on `dequeue`
...
# Dealing with errors in the `Stack` and `Queue` implementations
...
s.push(5)
s.pop()
# uncomment the next line to see a crash!
# s.pop()
```
...
**Key question**: Can we preserve the features of the `Deque` while avoiding
the linear time complexity of `addfirst` and `removefirst`?
