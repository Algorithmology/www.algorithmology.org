...
- What is the purpose of the `assert` and `print` statements?
...
- {{< iconify fa6-solid lightbulb >}} **Key Insights**:
  - **Double Time**: Execution time doubles when input size doubles
    - Indicates *linear time complexity* or $O(n)$.
  - **Quadruple Time**: Time quadruples when input size doubles
    - Indicates *quadratic time complexity* or $O(n^2)$.
  - **Eight-times Time**: Time increases eight-fold when input size doubles
    - Indicates *cubic time complexity* or $O(n^3)$
...
- Determine which function is best according to time overhead
