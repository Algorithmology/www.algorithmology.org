...
As you browse the resources on this site you will notice that it contains both
Python source code and the output from running code when the suite was rendered.
For instance, here is the source code for a `duplicates` function that uses a
double-nested `for` loop to determine whether or not the `input_list` contains
a duplicate `int` value.
...
def timetrials(function: Callable, n: int, trials: int = 10) -> None:
    """Time the provided function with an input of size n for trials number of times."""
    totaltime = 0
