---
title: Linked and Doubly Linked Lists
description: "Investigate List-Based Abstract Data Types"
date: 2024-03-11
layout: full
toc: true
toc-expand: 3
---

## Exploration

- {{< iconify fa6-solid book-open size=18px >}} Read chapters 7 and 8 in [A
First Course on Data Structures in
Python](https://donsheehy.github.io/datastructures/)
- {{< iconify fa6-brands github size=18px >}} Try out the implementations of
`OrderedList` in the `ds2` package of
[donsheehy/datastructures](https://github.com/donsheehy/datastructures/). Can
you write your own tests cases for these abstract data types? Can you use these
abstract data types in your own program?
- {{< iconify fa6-solid book size=18px >}} Note that, along with chapter 8,
chapter 7 will again be the focus for next week

## Activities

- {{< iconify fa6-solid people-line size=18px >}} **Monday and Wednesday**:
Linked Lists and Doubly Linked Lists
  - Deepen understanding of an abstract data type (ADT)
  - Understand implementation trade-offs for list-based structures
  - Use asymptotic analysis to characterize performance of an ADT
  - Examine Python source code segments to illustrate key points
- {{< iconify fa6-solid diagram-project size=18px >}} **Thursday**: Mid-term executable examination
- {{< iconify game-icons team-idea size=18px >}} **Friday**: Start new algorithm all-hands project

## Project

### Goal

Complete an executable examination for the mid-term exam that covers the content
in Chapters one through five of [A First Course on Data Structures in
Python](https://donsheehy.github.io/datastructures/). In the following week you
will receive an additional assignment that invites you to write a retrospective
of one of the Python functions that you implemented as part of the executable
examination.

## Slides

```{=html}
<iframe class="slide-deck" src="../../slides/weeknine/"></iframe>
```

{{< iconify majesticons presentation-play size=18px >}} Full Screen: [Week Nine: Implementing List-Based Structures](../../slides/weeknine/)
